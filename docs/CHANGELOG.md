...
- **Long-press multi-select, marquee selection & grouping:** hold one finger still on the canvas (~0.5 s) to enter multi-select mode (a small hint appears at the bottom-left), then:
  - **slide the held finger to draw a marquee** over the nodes — the nodes it sweeps are selected live (the same marquee helpers the mouse uses), and
  ...
  - **hold a node** (its body, not a port) to open that node's existing options menu — the same one the mouse reaches with right-click: *Edit Node Code*, *Edit Formula & Inputs*, draw order, *Group* / *Ungroup*, *View Calculation Trace*, *Delete Node*. Dragging the held finger dismisses the menu and moves the node, so a pause before a drag stays a drag.
